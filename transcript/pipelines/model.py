...
class AVSR(torch.nn.Module):
    def __init__(self, modality, model_path, model_conf, rnnlm=None, rnnlm_conf=None,
        ...
        if labels_type == "char":
            ...
            word.split()[0]
            for word in open(file_path, encoding="utf-8").read().splitlines()
        ] + ['<eos>']
        self.odim = len(self.token_list)
        self.model = E2E(self.odim, self.train_args)
        # These downloaded files contain a tensor-only state dict. Loading in
        # weights-only mode avoids unsafe pickle deserialization and the
        # PyTorch FutureWarning emitted by the old implicit default.
